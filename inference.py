"""
inference.py — Canlı Tahmin ve Sinyal Üretimi (Inference)

Görevi:
  1. Eğitilmiş model (.pth) ve MinMaxScaler (.pkl) objelerini yüklemek.
     (Dosyalar yoksa train.py çağrılarak arka planda yeniden üretilir.)
  2. ccxt ile BTC/USDT'nin ANLIK son 60 mumunu (1h) çekip ölçeklemek.
  3. Model tahminini alıp inverse_transform ile gerçek USDT fiyatına çevirmek
     ve %0.2 eşik (threshold) mantığıyla AL / SAT / BEKLE sinyali üretmek.

NOT: Bu dosya SADECE tahmin ve sinyal üretir. Canlı emir (execution)
ve API key yönetimi kapsam dışıdır. Tüm veri GERÇEK piyasa verisidir.
"""

import os

import joblib
import numpy as np
import torch

from data_pipeline import fetch_ohlcv, OHLCV_COLUMNS
from model import TradeAILSTM
import train  # sabitler ve yeniden-eğitim fallback'i için


# Sinyal eşiği: tahmin, mevcut fiyattan bu oranın üstünde/altında ise işlem sinyali
THRESHOLD = 0.002  # %0.2

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def load_model_and_scaler():
    """
    Eğitilmiş modeli ve scaler'ı yükler.

    Eğer artefakt dosyaları (.pth / .pkl) mevcut değilse, train.py'deki
    eğitim fonksiyonu çağrılarak model ve scaler yeniden üretilir
    (gerçek veriyle fit edilir) ve diske kaydedilir.
    """
    if os.path.exists(train.MODEL_PATH) and os.path.exists(train.SCALER_PATH):
        print("[YÜKLE] Kayıtlı model ve scaler bulundu, yükleniyor...")
        scaler = joblib.load(train.SCALER_PATH)
        model = TradeAILSTM(
            input_size=train.INPUT_SIZE,
            hidden_size=train.HIDDEN_SIZE,
            num_layers=train.NUM_LAYERS,
            output_size=train.OUTPUT_SIZE,
        ).to(DEVICE)
        model.load_state_dict(torch.load(train.MODEL_PATH, map_location=DEVICE))
    else:
        print("[YÜKLE] Kayıt bulunamadı -> train.py ile yeniden eğitiliyor...")
        model, scaler = train.train()  # gerçek veriyle eğit + kaydet
        model = model.to(DEVICE)

    model.eval()
    return model, scaler


def fetch_latest_window(symbol="BTC/USDT", timeframe="1h", sequence_length=None):
    """
    Anlık son `sequence_length` mumu çeker ve ham OHLCV matrisini döndürür.

    Dönüş
    -----
    window_raw : numpy.ndarray, şekil (sequence_length, n_features)
    last_close : float  (en son mumun gerçek kapanış fiyatı, USDT)
    """
    if sequence_length is None:
        sequence_length = train.SEQUENCE_LENGTH

    # Kenar durumlarına karşı biraz fazla mum çekip son N tanesini alıyoruz
    df = fetch_ohlcv(symbol=symbol, timeframe=timeframe, limit=sequence_length + 5)
    window_raw = df[OHLCV_COLUMNS].values[-sequence_length:]
    last_close = float(window_raw[-1, train.TARGET_COL_INDEX])
    return window_raw, last_close


def predict_price(model, scaler, window_raw):
    """
    Ham 60 mumluk pencereyi ölçekler, modele verir ve tahmin edilen
    kapanış fiyatını gerçek USDT değerine (inverse_transform) çevirir.
    """
    # 1) Ölçekle (scaler 5 özellik üzerine fit edilmiştir)
    window_scaled = scaler.transform(window_raw)

    # 2) LSTM girdisine dönüştür: (1, sequence_length, n_features)
    x = torch.tensor(window_scaled, dtype=torch.float32).unsqueeze(0).to(DEVICE)

    # 3) Tahmin (ölçekli, 0-1 aralığında bir 'close' değeri)
    with torch.no_grad():
        pred_scaled = model(x).cpu().numpy().item()

    # 4) inverse_transform: scaler tüm OHLCV için tanımlı olduğundan,
    #    dummy bir satır kurup yalnızca 'close' sütununa tahmini yerleştiriyoruz.
    dummy = np.zeros((1, len(OHLCV_COLUMNS)))
    dummy[0, train.TARGET_COL_INDEX] = pred_scaled
    predicted_price = float(scaler.inverse_transform(dummy)[0, train.TARGET_COL_INDEX])

    return predicted_price


def generate_signal(current_price, predicted_price, threshold=THRESHOLD):
    """
    Eşik (threshold) mantığıyla sinyal üretir.

    Dönüş
    -----
    signal : int   ( 1 = AL, -1 = SAT, 0 = BEKLE )
    label  : str
    change : float (oransal değişim)
    """
    change = (predicted_price - current_price) / current_price

    if change > threshold:
        return 1, "AL", change
    elif change < -threshold:
        return -1, "SAT", change
    else:
        return 0, "BEKLE", change


if __name__ == "__main__":
    SYMBOL = "BTC/USDT"
    TIMEFRAME = "1h"

    # 1) Model + scaler
    model, scaler = load_model_and_scaler()

    # 2) Anlık son 60 mum
    print(f"\n[VERİ] {SYMBOL} son {train.SEQUENCE_LENGTH} mum çekiliyor...")
    window_raw, current_price = fetch_latest_window(symbol=SYMBOL, timeframe=TIMEFRAME)

    # 3) Tahmin + sinyal
    predicted_price = predict_price(model, scaler, window_raw)
    signal, label, change = generate_signal(current_price, predicted_price)

    # ----------------------- Sonuç -----------------------
    print("\n" + "=" * 45)
    print(f"  Sembol              : {SYMBOL} ({TIMEFRAME})")
    print(f"  Şu anki Fiyat       : {current_price:,.2f} USDT")
    print(f"  Tahmin Edilen Fiyat : {predicted_price:,.2f} USDT")
    print(f"  Beklenen Değişim    : {change * 100:+.3f} %  (eşik: ±{THRESHOLD * 100:.1f}%)")
    print(f"  Üretilen Sinyal     : {label} ({signal})")
    print("=" * 45)
